    ...
    <div className="fade-in">
      ...
      <div className="bg-white border border-[#E8E6DF] rounded-2xl p-5 mb-4">
        ...
        <div className="flex gap-2 flex-wrap items-center">
          <select value={form.wardNo} onChange={(e) => setForm({ ...form, wardNo: e.target.value })} className={selectClass}>
            ...
            {wards.map((w) => (
              <option key={w.wardNo} value={w.wardNo}>Ward {w.wardNo} — {w.wardType}</option>
            ))}
          </select>
          <select value={form.patientId} onChange={(e) => setForm({ ...form, patientId: e.target.value })} className={selectClass}>
            <option value="">Select Patient</option>
            {patients.map((p) => (
              <option key={p.patientId} value={p.patientId}>#{p.patientId} — {p.patientName}</option>
            ))}
          </select>
          <select value={form.doctorName} onChange={(e) => setForm({ ...form, doctorName: e.target.value })} className={selectClass}>
            <option value="">Select Doctor</option>
            {doctors.map((d) => (
              <option key={d.doctorName} value={d.doctorName}>{d.doctorName} — {d.specialization}</option>
            ))}
          </select>
      ...
      <div className="bg-white border border-[#E8E6DF] rounded-2xl overflow-hidden">
        <div className="px-5 py-4 border-b border-[#E8E6DF] flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <span className="text-xs text-[#9CA3AF] bg-[#F4F3EE] px-2.5 py-1 rounded-full">
              {admissions.filter(a => !a.dischargedAt).length} active
            </span>
            <span className="text-xs text-[#9CA3AF] bg-[#F4F3EE] px-2.5 py-1 rounded-full">
              {admissions.filter(a => a.dischargedAt).length} discharged
            </span>
          </div>
        </div>
        <table className="w-full text-sm">
          ...
          <tbody>
            {admissions.length === 0 && (
              <tr><td colSpan={8} className="text-center text-[#C4C2BB] py-12 text-sm">No admissions found</td></tr>
            )}
            {admissions.map((a, i) => (
              <tr key={a.admissionId}
                className={`border-b border-[#F4F3EE] hover:bg-[#FAFAF7] transition ${i === admissions.length - 1 ? "border-0" : ""}`}>
                ...
                <td className="px-4 py-3.5 text-[#9CA3AF] text-xs">
                  {a.admittedAt ? new Date(a.admittedAt).toLocaleString() : "—"}
                </td>
                ...
                <td className="px-4 py-3.5 text-right">
                  {!a.dischargedAt
                    ? <button onClick={() => handleDischarge(a.admissionId)}
                        className="text-xs text-[#0D6E63] hover:text-white hover:bg-[#0D6E63] border border-[#0D6E63]/30 hover:border-[#0D6E63] px-3 py-1.5 rounded-lg transition font-medium">
                        Discharge
                      </button>
